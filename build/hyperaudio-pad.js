/*! hyperaudio-pad v0.7.3 ~ (c) 2012-2018 Hyperaudio Inc. <hello@hyperaud.io> (http://hyperaud.io) http://hyperaud.io/licensing/ ~ Built: 3rd December 2018 15:02:38 */
      ...
      return {
        ...
        _setup: function(track) {
          ...
          if (
            ...
            track.target !== 'Transcript-container'
          ) {
...
var hyperaudio = (function() {
      ...
      if ((options = arguments[i]) != null) {
        ...
        for (name in options) {
          ...
          if (
            deep &&
            copy &&
            (hyperaudio.isPlainObject(copy) || (copyIsArray = hyperaudio.isArray(copy)))
          ) {
            if (copyIsArray) {
              copyIsArray = false;
  ...
  hyperaudio.extend({
    ...
    getURLParameter: function(name) {
      ...
      return (
        decodeURIComponent(
          (new RegExp('[?|&]' + name + '=' + '([^&;]+?)(&|#|;|$)').exec(win.location.search) || [
            ,
            ''
          ])[1].replace(/\+/g, '%20')
        ) || null
...
var EditBlock = (function(document) {
  ...
  EditBlock.prototype.cancel = function(e) {
    ...
    if (
      target == this.el ||
      target.parentNode == this.el ||
      target.parentNode.parentNode == this.el
    ) {
      return;
    }
  ...
  function SideMenu(options) {
    ...
    this.el =
      typeof this.options.el == 'string'
        ...
        : this.options.el;
    this.transcripts =
      typeof this.options.transcripts == 'string'
        ? document.querySelector(this.options.transcripts)
        : this.options.transcripts;
    this.music =
      typeof this.options.music == 'string'
        ? document.querySelector(this.options.music)
        : this.options.music;
...
*/
    ...
    var addTestFolders = function() {
      ...
      testChild[testChild.length - 1].push(
        self.makeMenuFolder(testFolder[testFolder.length - 1], 'Child Folder 1 -> 1')
      );
      testChild[testChild.length - 1].push(
        self.makeMenuFolder(testFolder[testFolder.length - 1], 'Child Folder 1 -> 2')
      );
      testChild[testChild.length - 1].push(
        self.makeMenuFolder(testFolder[testFolder.length - 1], 'Child Folder 1 -> 3')
      );
      ...
      // Add some child folders to the folder
      testChild[testChild.length - 1].push(
        self.makeMenuFolder(testFolder[testFolder.length - 1], 'Child Folder 2 -> 1')
      );
      testChild[testChild.length - 1].push(
        self.makeMenuFolder(testFolder[testFolder.length - 1], 'Child Folder 2 -> 2')
      );
      testChild[testChild.length - 1].push(
        self.makeMenuFolder(testFolder[testFolder.length - 1], 'Child Folder 2 -> 3')
      );
  ...
  SideMenu.prototype.initMusic = function() {
    ...
    function onDrop(el) {
      ...
      var html =
        '<form><div><span class="icon-music">' +
        title +
        '</span></div>' +
  ...
  SideMenu.prototype.toggleFolder = function(e) {
    ...
    if (folder) {
      ...
      hyperaudio.gaEvent({
        ...
        action:
          'togglefolder: ' +
          (hyperaudio.hasClass(folder, 'open') ? 'Opened' : 'Closed') +
          ' -> ' +
          name
      });
...
var WordSelect = (function(window, document, hyperaudio) {
  ...
  WordSelect.prototype.move = function(e) {
    ...
    if (
      Math.abs(point.pageX - this.startX) < this.options.threshold &&
  ...
  return {
    init: function(options) {
      ...
      if (rights) {
        ...
        window.top.document.documentElement.addEventListener(
          'keydown',
          function(event) {
    ...
    onKeyDown: function(event) {
      ...
      if (!this.enabled || !this.value || !(event.ctrlKey || event.metaKey)) {
        if (DEBUG)
          console.log(
            '[onKeyDown] : Exit | enabled = ' + this.enabled + ' | value = "' + this.value + '"'
          );
        return;
      }
      ...
      if (DEBUG)
        console.log('[onKeyDown] : Textarea prepared for copy | value = "' + this.value + '"');
  ...
  Music.prototype = {
    create: function() {
      ...
      if (this.target) {
        ...
        if (this.options.media.mp3 || this.options.media.mp4) {
          // Assumes we have the ogg
          this.load();
        }
      } else {
  ...
  Player.prototype = {
    create: function() {
      ...
      if (this.target) {
        ...
        this.solution.html.addEventListener(
          'progress',
          function(e) {
            if (this.readyState > 0) {
              self.commandsIgnored = false;
        ...
        if (tryLoad) {
          // We have something - try to load it.
          this.load();
        }
      } else {
        this._error('Target not found : ' + this.options.target);
        ...
        this.target.addEventListener(
          ...
          function(event) {
            ...
            if (event.target.nodeName.toLowerCase() === opts.word) {
              ...
              self._trigger(hyperaudio.event.userplayword, {
                msg: 'User clicked on a word to play from'
              });
            }
          },
          false
        ...
        this.textSelect = new hyperaudio.WordSelect({
          ...
          onDragStart: function(e) {
            if (opts.stage) {
              ...
              var html =
                this.getSelection().replace(/ class="[\d\w\s\-]*\s?"/gi, '') +
          ...
          if (this.options.mix.url) {
            hyperaudio.xhr({
              ...
              complete: function(event) {
                ...
                self.initDragDrop();
                self._trigger(hyperaudio.event.load, {
                  msg: 'Loaded "' + self.options.mix.url + '"'
                });
              },
              error: function(event) {
                ...
                self._error(
                  this.status + ' ' + this.statusText + ' : "' + self.options.mix.url + '"'
                );
              }
            });
  ...
  Projector.prototype = {
    ...
    load: function(index) {
      ...
      if (activePlayer !== false) {
        ...
        if (DEBUG)
          console.log('[Projector|load] player[%d].load() | initPopcorn()', this.activePlayer);
      }
      ...
      if (DEBUG)
        console.log(
          '[Projector|load] contentIndex: %d | activePlayer: %d',
          this.contentIndex,
          this.activePlayer
        );
    },
      ...
      if (media && this.player.length > 1) {
        ...
        if (prepared === false) {
          ...
          if (this.player[this.nextPlayer]) {
            ...
            if (DEBUG)
              console.log('[Projector|prepare] player[%d].load() | initPopcorn()', this.nextPlayer);
            if (DEBUG)
              console.log(
                '[Projector|prepare] prepared=false | nextPlayer: %d | alignStart: %d',
                this.nextPlayer,
                alignStart
              );
          }
          ...
          if (prepared !== this.activePlayer) {
            ...
            if (DEBUG)
              console.log(
                '[Projector|prepare] prepared=' + prepared + ' | nextPlayer: %d | alignStart: %d',
                this.nextPlayer,
                alignStart
              );
          }
...
*/
    ...
    play: function() {
      ...
      if (DEBUG)
        console.log(
          '[Projector|play] resume = ' + resume + ' | content.length = %d | arguments = %o',
          this.content.length,
          arguments
        );
      ...
      if (this.stage && this.stage.target) {
        ...
        for (i = 0, len = this.content.length; i < len; i++) {
          if (this.content[i].element === sectionElem) {
            ...
            this._trigger(hyperaudio.event.userplayword, {
              msg: 'User clicked on a word to play from'
            });
            this.play(jumpTo);
            break;
    ...
    updateContent: function() {
      ...
      if (this.stage && this.stage.target) {
        ...
        if (this.needsInitVideo && this.content.length) {
          ...
          this.isReadyToPlay = false;
          if (DEBUG)
            console.log('[Projector|updateContent#2] isReadyToPlay = ' + this.isReadyToPlay);
        }
      }
    },
      ...
      for (i = 0, iLen = this.content.length; i < iLen; i++) {
        ...
        for (e = 0, eLen = effect.length; e < eLen; e++) {
          if (i < jumpTo.contentIndex) {
            ...
            effect[e].init = false;
          } else if (effect[e].type === 'fadeOut') {
            // Need an isEndEffect() method
            effect[e].init = false;
          } else {
  ...
  function updateGUI() {
    ...
    if (shareTextElem) {
      var shareText =
        '<iframe src="' +
        myUrls.share +
        '" height="294" width="480" frameborder="0" scrolling="no" allowfullscreen seamless></iframe>';
  ...
  function loaded() {
    ...
    if (!HAP.options.viewer || transcriptId || mixId) {
      ...
      if (!HAP.options.viewer || (transcriptId && !mixId)) {
        ...
        if (HAP.options.viewer) {
          transcript.target.addEventListener(
            HA.event.load,
            function(e) {
    ...
    if (!HAP.options.viewer) {
      ...
      fade = new HA.DragDrop({
        ...
        onDrop: function(el) {
          ...
          el.className += ' effect';
          el.innerHTML =
            '<form onsubmit="return false"><label>Fade Effect: <span class="value">1</span>s</label><input id="effect-duration" type="range" value="1" min="0.5" max="5" step="0.1" onchange="this.setAttribute(\'value\', this.value); this.previousSibling.querySelector(\'span\').innerHTML = this.value;"></form>';
      ...
      trim = new HA.DragDrop({
        ...
        onDrop: function(el) {
          ...
          el.className += ' effect';
          el.innerHTML =
            '<form onsubmit="return false"><label>Trim: <span class="value">1</span>s</label><input id="effect-duration" type="range" value="1" min="0" max="5" step="0.1" onchange="this.setAttribute(\'value\', this.value); this.previousSibling.querySelector(\'span\').innerHTML = this.value;"></form>';
    ...
    if (shareTextElem) {
      shareTextElem.addEventListener(
        'click',
        function(e) {
          e.preventDefault();
          shareTextElem.focus();
